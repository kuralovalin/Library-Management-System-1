public class QuickSort {
    ...
    private static <T extends Comparable<T>> int partition(T[] table, int first, int last) {
        ...
        do {
            ...
            while ((up < last) && (((Book)pivot).compareTime((Book)table[up]) >= 0)) {
                up++;
            }
            // assert: up equals last or table[up] > pivot.
            while (((Book)pivot).compareTime((Book)table[down]) < 0) {
                down--;
            }
     ...
     */
    private static <T extends Comparable<T>> void sortFL(T[] table, int first, int last) {
        int middle = (first + last) / 2;
        /* Sort table[first], table[middle], table[last]. */
        if (((Book)table[middle]).compareTime((Book)table[first]) < 0) {
            swap(table, first, middle);
        }
        // assert: table[first] <= table[middle]
        if (((Book)table[last]).compareTime((Book)table[middle]) < 0) {
     ...
     */
    protected  boolean verify(Comparable[] test) {
        boolean ok = true;
        int i = 0;
        while (ok && i < test.length-  1) {
            ok = ((Book)test[i]).compareTime((Book)test[i + 1]) <= 0;
            i++;
        }
        return ok;
    }
