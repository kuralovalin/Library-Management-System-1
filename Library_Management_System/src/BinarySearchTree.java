...
public class BinarySearchTree<E extends Comparable<E>> extends BinaryTree<E> implements SearchTree<E> {
     ...
     * @return String which represents avl tree
     */
    ...
    /**
     * Convert Tree as List
     *
     * @return list
     */
    public List inorderList() {
        List<E> sb = new ArrayList();
        inOrderSearch(root, sb);
        return sb;
    }

    /**
     * Perform an inorder traversal,and add to datas to List
     *
     * @param node The local root
     * @param sb   The List for convert to list datas
     */
    private void inOrderSearch(Node<E> node, List<E> sb) {
        if (node == null) {
            //do nothing
        } else {
            inOrderSearch(node.left, sb);
            sb.add(node.data);
            inOrderSearch(node.right, sb);
        }
    }
    ...
    /**
     * Method to check avl tree contain element ot not
    ...
    /**
     * Starter method find for bookID
     * pre: The target object must implement the Comparable interface.
     *
     * @param target The Comparable object being sought
     * @return The object if found, otherwise null
     */
    public E find(String target) {
        return find(root, target);
    }

    /**
     * Recursive find method for bookID
     *
     * @param localRoot The local subtree's root
     * @param target    The object being sought
     * @return The object, if found, otherwise null
     */
    private E find(Node<E> localRoot, String target) {

        if (localRoot == null)
            return null;
        // Compare the target with the data field at the root.
        if (((Book) localRoot.data).compareID(target) == 0) {
            return localRoot.data;
        }
        else if(((Book) localRoot.data).compareID(target) < 0 ) {
    ...
    /**
     * Method removes element from avl tree
